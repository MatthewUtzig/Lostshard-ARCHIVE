...
public class Output {
	...
	public static void plotInfo(Player player, Plot plot) {
		...
		player.sendMessage(ChatColor.YELLOW + "Protected: " + ChatColor.WHITE
				+ Utils.booleanToString(plot.isProtected()) + ChatColor.YELLOW
				+ ", Status: " + ChatColor.WHITE + "private/public"
				+ ChatColor.YELLOW + ", Alignment: " + ChatColor.WHITE
				+ "Lawfull/Criminal");
		player.sendMessage(ChatColor.YELLOW + "Allow Exploosions: "
				+ ChatColor.WHITE
				+ Utils.booleanToString(plot.isAllowExplosions()));
