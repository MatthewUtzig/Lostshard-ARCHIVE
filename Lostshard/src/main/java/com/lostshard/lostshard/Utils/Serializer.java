...
public class Serializer {
	...
	public static Location deserializeLocation(String locationString) {
		...
		try {
			...
			deLoc = new Location(Bukkit.getWorld(locationList[0]),
					Double.parseDouble(locationList[1]),
					Double.parseDouble(locationList[2]),
					Double.parseDouble(locationList[3]),
					Float.parseFloat(locationList[4]),
					Float.parseFloat(locationList[5]));
		} catch (Exception e) {
			Lostshard.logger.log(Level.WARNING,
					"[SERIALIZER] Exception while serializing a location.");
		}
		return deLoc;
	}
