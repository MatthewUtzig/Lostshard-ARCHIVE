	...
	private void pay(CommandSender sender, String[] args) {
		...
		if(sender instanceof Player) {
			...
			if(pPlayer.getMoney() < amount) {
				Output.simpleError(player, "You cannot affort to pay "+targetPlayer.getName()+" "+amount+"gc.");
				return;
			}
			pPlayer.subtractMoney(amount);
		}
		tpPlayer.addMoney(amount);
		sender.sendMessage(ChatColor.GOLD+"You have paied "+targetPlayer.getName()+" "+amount+"gc.");
		Output.simpleError(targetPlayer, sender.getName()+"has paied you "+amount+"gc.");
	}
		...
		for(NPC npc : NPCHandler.getBankers())
			if(Utils.isWithin(player.getLocation(), npc.getLocation(), Variables.bankRadius)){
				...
				if(player.getInventory().containsAtLeast(new ItemStack(Material.GOLD_INGOT), amount)) {
					...
					player.getInventory().remove(new ItemStack(Material.GOLD_INGOT, amount));
					Output.positiveMessage(player, "You have traded "+amount+" gold ingots into "+amount*Variables.goldIngotValue+" gc.");
				}else
