...
public class SPL_PermanentGateTravel extends Spell {
	public SPL_PermanentGateTravel(Scroll scroll) {
		super(scroll);
		this.setPrompt("What rune would you like to open a permanent gate to?");
	}
	...
	public void doAction(Player player) {
		...
		if (plot == null || !plot.isPrivatePlot()
			...
			final Block destBlock = runeLoc.getWorld().getBlockAt(
					runeLoc.getBlockX(), runeLoc.getBlockY(),
					runeLoc.getBlockZ());
			final Block srcBlock = player.getWorld().getBlockAt(
					loc.getBlockX(), loc.getBlockY(), loc.getBlockZ());
			final Block extraDestBlock = runeLoc.getWorld().getBlockAt(
					runeLoc.getBlockX(), runeLoc.getBlockY() + 1,
					runeLoc.getBlockZ());
			final Block extraSrcBlock = player.getWorld().getBlockAt(
			...
			if (destBlock.getRelative(0, -1, 0).getType()
					.equals(Material.LAPIS_BLOCK)
					|| destBlock.getRelative(0, -2, 0).getType()
							.equals(Material.LAPIS_BLOCK)) {
				Output.simpleError(player,
						"can't gate to a Lapis Lazuli block.");
				return;
			}
			...
			final int yaw = Math.round(Math
					.abs(player.getLocation().getYaw() / 90));
			boolean direction;
			if (yaw == 0 || yaw == 2)
				direction = true;
			else
			...
			Output.simpleError(player,
					"can't gate to there, not a friend of the plot.");
	}
	...
	public void preAction(Player player) {
		Output.positiveMessage(player,
				"You begin casting Permanent Gate Travel...");
	}
