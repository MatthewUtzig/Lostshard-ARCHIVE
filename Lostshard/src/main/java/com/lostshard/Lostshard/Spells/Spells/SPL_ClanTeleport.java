	...
	public void doAction(Player player) {
		...
		if (targetPlayer == null) {
			...
			if (playerClan != null
					...
					&& playerClan.getName()
							.endsWith(targetPlayerClan.getName())) {
				if (targetPseudoPlayer.isPrivate()) {
					Output.positiveMessage(player,
							"Can't teleport to that player, they are set to private.");
					return;
				}

				if (targetPlayer.getHealth() <= 0) {
					Output.positiveMessage(player,
							"You can't teleport to that player, as they are dead.");
					return;
				}

				// check for lapis below you
				if (player.getLocation().getBlock().getRelative(0, -1, 0)
						.getType().equals(Material.LAPIS_BLOCK)
						|| player.getLocation().getBlock()
								.getRelative(0, -2, 0).getType()
								.equals(Material.LAPIS_BLOCK)) {
					Output.simpleError(player,
							"can't teleport from a Lapis Lazuli block.");
					return;
				}

				// check for lapis below your target location
				if (targetPlayer.getLocation().getBlock().getRelative(0, -1, 0)
						.getType().equals(Material.LAPIS_BLOCK)
						|| targetPlayer.getLocation().getBlock()
								.getRelative(0, -2, 0).getType()
								.equals(Material.LAPIS_BLOCK)) {
					Output.simpleError(player,
							"can't teleport to a Lapis Lazuli block.");
					return;
				}
				...
				Output.sendEffectTextNearby(player,
