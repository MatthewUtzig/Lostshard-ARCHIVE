...
public class foodHealHandler {
	...
	public static void foodHeal(PlayerInteractEvent event) {
		...
		if(foodType.getItem() == Material.PORK) {
			int curSkill = pPlayer.getCurrentBuild().getLumberjacking().getLvl();
			if(curSkill > 500) {
				curHealth += 5;
			}
		}else if(foodType.getItem() == Material.COOKED_FISH) {
			int curSkill = pPlayer.getCurrentBuild().getFishing().getLvl();
			if(curSkill > 750) {
				curHealth += 2;
			}
		}
		...
		int survSkill = pPlayer.getCurrentBuild().getSurvivalism().getLvl();
