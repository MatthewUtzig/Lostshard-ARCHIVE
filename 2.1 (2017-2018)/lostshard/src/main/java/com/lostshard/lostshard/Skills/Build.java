...
public class Build {
	...
	@AttributeOverrides({ @AttributeOverride(name = "lvl", column = @Column(name = "lumberjacking_level") ),
	...
	@AttributeOverrides({ @AttributeOverride(name = "lvl", column = @Column(name = "fishing_level") ),
			@AttributeOverride(name = "locked", column = @Column(name = "fishing_locked") ) })
	private FishingSkill fishing = new FishingSkill();

	@AttributeOverrides({ @AttributeOverride(name = "lvl", column = @Column(name = "survivalism_level") ),
	...
	@AttributeOverrides({ @AttributeOverride(name = "lvl", column = @Column(name = "taming_level") ),
			@AttributeOverride(name = "locked", column = @Column(name = "taming_locked") ) })
	private TamingSkill taming = new TamingSkill();

	@AttributeOverrides({ @AttributeOverride(name = "lvl", column = @Column(name = "magery_level") ),
			@AttributeOverride(name = "locked", column = @Column(name = "magery_locked") ) })
	private MagerySkill magery = new MagerySkill();

	@AttributeOverrides({ @AttributeOverride(name = "lvl", column = @Column(name = "archery_level") ),
	...
	public Skill getSkillByName(String name) {
		...
		case "lumberjacking":
			return this.getLumberjacking();
		case "blacksmithy":
			return this.getBlackSmithy();
		case "blades":
			return this.getBlades();
		case "archery":
			return this.getArchery();
		case "brawling":
			return this.getBrawling();
		case "fishing":
			return this.getFishing();
		case "survivalism":
			return this.getSurvivalism();
		case "taming":
			return this.getTaming();
		case "magery":
			return this.getMagery();
		default:
			return null;
		}
	}
	...
	public List<Skill> getSkills() {
		...
		skills.sort((skill1, skill2) -> skill1.getName().compareTo(skill2.getName()));
		return skills;
	}
