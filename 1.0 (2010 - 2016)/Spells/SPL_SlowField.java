...
public class SPL_SlowField extends Spell {
	private static final String 	_name = "Slow Field";
	private static final String 	_spellWords = "Webicus Fieldicus";
	...
	public void doAction(Player player) {
		...
		for(int x = _blockFound.getX() - 3; x <= _blockFound.getX()+2; x++) {
			for(int y = _blockFound.getY() - 3; y <= _blockFound.getY()+2; y++) {
				for(int z = _blockFound.getZ() - 3; z <= _blockFound.getZ()+2; z++) {
					...
					if(!blockAt.getType().equals(Material.AIR)) {
						...
						if(blockAbove.getType().equals(Material.AIR) || blockAbove.getType().equals(Material.SNOW)) {
							if(Utils.isWithin(blockAbove.getLocation(), _blockFound.getLocation(), 3))
		...
		if(blocks.size() > 0) {
			WebTrap fireField = new WebTrap(blocks, player.getName(), 50);
