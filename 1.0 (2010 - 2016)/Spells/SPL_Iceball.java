...
public class SPL_Iceball extends Spell {
	private static final String 	_name = "Ice Ball";
	private static final String 	_spellWords = "Freezius Ballicus";
	private static final int 		_castingDelay = 0;
	private static final int 		_cooldownTicks = 20;
	private static final int		_manaCost = 25;
	private static final int[]		_reagentCost = {287};
	...
	private boolean _isProtPlot = false;
	...
	public boolean verifyCastable(Player player, PseudoPlayer pseudoPlayer) {
		...
		for(Plot p : plots) {
			// if the iceball was cast within 10 blocks of a plot
			if(Utils.distance(p.getLocation(), _blockFound.getLocation()) < p.getRadius()+10) {
				// that is protected
				if(p.isProtected()) {
					// and the caster is not a friend of the plot
					if(!p.isMember(player.getName())) {
						_isProtPlot = true;//Output.simpleError(player, "You cannot cast "+_name+" there, that plot is protected.");
						return true;
					}
				}
			}
	...
	public void doAction(Player player) {
		...
		for(int x = _blockFound.getX() - 3; x <= _blockFound.getX()+3; x++) {
			for(int y = _blockFound.getY() - 3; y <= _blockFound.getY()+3; y++) {
				for(int z = _blockFound.getZ() - 3; z <= _blockFound.getZ()+3; z++) {
					...
					if(Utils.isWithin(blockAt.getLocation(), _blockFound.getLocation(), 3)) {
						if(_isProtPlot) {
							...
							int blockTypeId = blockAt.getTypeId();
							if((blockTypeId == 8 || blockTypeId == 9) && blockAt.getWorld().getBlockTypeIdAt(blockAt.getX(), blockAt.getY()+2, blockAt.getZ()) == 0)
								blockAt.setType(Material.ICE);
							else if(blockTypeId==10)
								blockAt.setType(Material.COBBLESTONE);
							else if(blockTypeId==11)
								blockAt.setType(Material.OBSIDIAN);
							else if(blockTypeId == 0 || blockTypeId == 51 || blockTypeId == 78) {
								blocks.add(blockAt);
							}
						}
					}
		...
		if(blocks.size() > 0) {
			Iceball iceball = new Iceball(blocks, player.getName(), 25, !_isProtPlot);
			Magery.addMagicStructure(iceball);
		}
	}
}
