...
public class EnderdragonHandler {
	...
	public static void respawnDragonCheck(PlayerChangedWorldEvent event) {
		...
		if (!spawnDrake)
			return;
		for (final Entity e : event.getPlayer().getLocation().getWorld().getEntities())
			if (e instanceof EnderDragon)
				e.remove();
		event.getPlayer().getLocation().getWorld().spawnEntity(event.getPlayer().getLocation().add(0, 40, 0),
